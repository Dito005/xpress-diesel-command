...
const fetchAdminDashboardData = async () => {
  ...
  return {
    activeJobs: {
      total: activeJobsCount || 0,
      inShop: inShopCount,
      roadCalls: roadCallsCount,
    },
    jobsPendingPayment: {
      total: pendingPaymentCount || 0,
    },
    revenueToday: {
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-full">
      <div className="flex-grow space-y-6 lg:w-2/3">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <KpiCard
            title="Active Jobs"
            value={data?.activeJobs.total.toString() || '0'}
            subtext={`In-shop: ${data?.activeJobs.inShop || 0} | Road: ${data?.activeJobs.roadCalls || 0}`}
            icon={Briefcase}
            isLoading={isLoading}
          />
          <KpiCard
            title="Jobs Pending Payment"
            value={data?.jobsPendingPayment.total.toString() || '0'}
          ...
          <KpiCard
            title="Revenue Today"
            value={`$${(data?.revenueToday.total || 0).toLocaleString()}`}
            subtext="Labor vs. Parts breakdown coming soon"
            icon={DollarSign}
            isLoading={isLoading}
          />
          <KpiCard
            title="Estimated Profit Today"
            value="$1,482"
            subtext="Based on current costs"
            icon={BarChart}
            isLoading={isLoading}
          />
        </div>
        ...
        <Card className="border">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              Live AI Warnings & Opportunities
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                ...
                <span>Job #A4B1C has been 'waiting_parts' for 3 days.</span>
              </li>
              <li className="flex items-start gap-2">
                <DollarSign className="h-4 w-4 mt-0.5 text-green-500 flex-shrink-0" />
                <span>Invoice #X9Y8Z seems underbilled for labor by 1.5 hours.</span>
              </li>
            </ul>
          </CardContent>
        </Card>
