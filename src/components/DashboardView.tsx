...
const fetchKpiData = async () => {
  ...
  const pendingInvoices = invoices.filter(i => ['unpaid', 'pending', 'draft'].includes(i.status));
  ...
  return {
    ...
    approvalsNeeded: {
      ...
      subtext: `2 need customer sign-off / 1 internal`,
    },
    techUtilization: {
...
export const DashboardView = ({ onInvoiceClick }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <KpiCard title="Active Jobs" metric={kpiData?.activeJobs.metric || '...'} subtext={kpiData?.activeJobs.subtext || '...'} />
        <KpiCard title="Jobs Pending Payment" metric={kpiData?.pendingPayments.metric || '...'} subtext={kpiData?.pendingPayments.subtext || '...'} />
        <KpiCard title="Approvals Needed" metric={kpiData?.approvalsNeeded.metric || '...'} subtext={kpiData?.approvalsNeeded.subtext || '...'} />
        <KpiCard title="Today’s Revenue" metric="$2,400" subtext="Labor: $1,800 / Parts: $600" />
        <KpiCard title="Estimated Profit" metric="$865" subtext="Includes shop fees / Excludes taxes" />
        <KpiCard title="Technician Utilization" metric={kpiData?.techUtilization.metric || '...'} subtext={kpiData?.techUtilization.subtext || '...'} />
      </div>
